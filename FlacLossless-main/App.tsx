...
import { X, Mic2, Disc, Sliders, Sparkles, Activity, Download, Zap, Key, List, Music, Wifi, Cookie } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [downloadError, setDownloadError] = useState<{ error: string; videoTitle?: string } | null>(null);
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden flex flex-col items-center justify-center transition-all duration-100" style={bgStyle}>
           ...
           <div className="flex justify-between items-center mb-8 relative z-20">
              ...
              <div className="flex gap-2 items-center">
                 ...
                 <button 
                    ...
                    className="p-1 px-2 rounded-full border border-green-500/30 text-[10px] bg-green-900/20 text-green-400 flex items-center gap-1 hover:bg-green-900/40 transition-colors"
                    title="Upload YouTube Cookies for authentication"
                 >
                    <Cookie size={10} /> COOKIES
                 </button>
                 <button 
      ...
      {showCookieUpload && (
        <div className="absolute inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-lg bg-[#0a0a0a] border border-green-500/30 rounded-3xl p-8 shadow-[0_0_100px_rgba(34,197,94,0.15)] relative overflow-hidden">
                <div className="absolute -top-20 -right-20 w-64 h-64 bg-green-600/20 rounded-full blur-3xl pointer-events-none"></div>

                <div className="flex justify-between items-center mb-6 relative z-10">
                    <h3 className="text-2xl font-bold text-white flex items-center gap-3">
                        <Cookie className="text-green-400" size={24} /> YouTube Cookies
                    </h3>
                    <button onClick={() => setShowCookieUpload(false)} className="text-gray-500 hover:text-white p-2 hover:bg-white/10 rounded-full transition-colors"><X size={20}/></button>
                </div>
                ...
                <div className="relative z-10">
                    <CookieUpload
                        onSuccess={() => {
                            ...
                            alert('✓ Cookies uploaded successfully! Try downloading again.');
                        }}
                        onError={(error) => {
                            console.error('Cookie upload error:', error);
                            alert('✗ Error uploading cookies: ' + error);
                        }}
                        showTitle={false}
                    />
                </div>
