...
class BackendService {
  ...
  async uploadCookies(file: File): Promise<{ success: boolean; message: string }> {
    try {
      ...
      const response = await fetch(getBackendUrl('/cookies'), {
        method: 'POST',
        body: formData
      });
      ...
      console.error('[Backend] Cookies upload failed:', e);
      throw e;
    }
  }
  ...
  async checkCookies(): Promise<{ has_cookies: boolean; path?: string; message?: string }> {
    try {
      ...
      throw new Error('Failed to check cookies');
    } catch (e) {
      console.error('[Backend] Cookies check failed:', e);
